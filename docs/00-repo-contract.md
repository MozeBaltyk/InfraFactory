...
Canonical artifact filenames (consumed by convention, not by explicit tfvars):
the cluster SSH keypair is `env/<PROVIDER>/<environment>/.key.pub` /
`.key.private` and the cluster token is `.token`, generated by the cluster
stack (`providers/shared/modules/ssh-keys`). The standalone OVH bastion reads
a served cluster's public key from that path by default (map key =
cluster workspace name); `public_key_file` is only an override.
