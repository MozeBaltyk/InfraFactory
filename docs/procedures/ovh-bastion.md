...
2. Deploy the cluster's network + keys (`ENV=<env> just ovh::bootstrap`),
   ...
   clusters = {
     <cluster> = {
       ...
       # public_key_file omitted → defaults to env/OVH/<cluster>/.key.pub
       # (key = cluster workspace name, generated by the cluster stack)
     }
   }
   ```

   `apply` hot-attaches one private NIC; the VM and its public IP are
   untouched. The NIC's guest name is not predictable (hot-attach) — the
   converge step renames it (MAC-matched) to the conventional iface.
...
2. Per cluster `<env>` (repeat for each):
   1. Create `env/OVH/<env>.tfvars` with its own `network.private.cidr`
      ...
      bastion. Run `ENV=<env> just ovh::bootstrap` (private network + SSH
      keys only), so the network and `env/OVH/<env>/.key.pub` exist.
   ...
   4. Set `bastion = { public_ip = "<same-ip>" }` in `env/OVH/<env>.tfvars`
...
* Treat keys as immutable once attached: the conventional iface name
  (`ens4`, `ens5`, …) follows *sorted key* order, so renaming a key
  renumbers interfaces (the converge role renames by MAC to these names).
