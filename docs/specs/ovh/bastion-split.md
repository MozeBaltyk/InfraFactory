...
* `clusters.<name> = { cidr, vlan_id, masters, workers }` in the bastion
  tfvars; `masters`/`workers` match the cluster tfvars (they size
  `PermitOpen`). The map key `<name>` MUST equal the cluster's
  workspace/`ENV` name.
* Cluster public key: generated by the cluster stack, read by default from
  `env/<PROVIDER>/<name>/.key.pub` (the shared `ssh-keys` artifact
  convention). `public_key_file` is only an override. Because the key is
  generated by the cluster, the cluster's keys + network targeted apply
  precedes this stack's attach.
...
* `converge` output feeds the Ansible role (username, host, per-cluster
  NIC iface/address/prefix/MAC, `permit_open`, sshd test addresses, both
  key lists).
...
* Bastion `ens3` is the public NIC (DHCP, first). Private NICs are
  hot-attached managed ports + interface attachments: attaching a cluster
  creates port + attach only, never a replacement. Hot-attached NICs do
  NOT enumerate predictably in the guest, so day-2 netplan matches the
  Neutron port MAC and renames to a conventional iface (`ens4`, `ens5`, …)
  via the converge role.
