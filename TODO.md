...
- [ ] Attach OVH-managed storage per role by key (`infra.masters/workers/vms.nfs`/`.object_storage`/`.block_storage`): automatic NFS client-mount resolution, per-role scoped S3 IAM credentials, per-VM block volume attachment
