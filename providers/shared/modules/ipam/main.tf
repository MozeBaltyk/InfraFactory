...
locals {
  ...
  master_ips = [
    ...
    cidrhost(var.cidr, var.host_offset_base + i)
  ]

  worker_ips = [
    for i in range(var.workers_count) :
    cidrhost(var.cidr, var.host_offset_base + var.masters_count + i)
  ]

  vm_ips = [
    for i in range(var.vms_count) :
    cidrhost(var.cidr, var.host_offset_base + var.masters_count + var.workers_count + i)
  ]
  ...
  last_node_hostnum = var.host_offset_base + local.node_count - 1
