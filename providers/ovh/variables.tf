...
variable "infra" {
  ...
  validation {
    ...
    error_message = "OVH requires at least one master node unless cluster.cloud_init_selected is \"default\", infra.workers.count is 0, and infra.vms.count is at least 1."
  }
...
variable "network" {
  ...
  validation {
    condition = (
      ...
      (
        var.cluster.cloud_init_selected == "default" &&
